...
$complaintData = [];
if ($complaintId) {
    $stmt = $conn->prepare("SELECT c.*, u.name, u.roll_no, u.class, u.section, u.sem, u.email,c.date_inc
                            FROM complaints c 
...
if (isset($_SESSION['dept_email'])) {
    ...
    $stmt = $conn->prepare("SELECT deptad_name FROM admin WHERE dept_email = ?");
    $stmt->bind_param("s", $dept_email);
    $stmt->execute();
    $result = $stmt->get_result();
    if ($result && $result->num_rows === 1) {
        $row = $result->fetch_assoc();
        $deptad_name = $row['deptad_name'];
    }
    $stmt->close();
}
?>
...
/* Modal styles for different screen sizes */
@media (max-width: 1024px) {
  .modal-content {
    min-width: 320px;
    max-width: 90vw;
...
<body>
<header>
   <img  id="mainLogo" src="images/uniResolveLogo.png" alt="Logo">
  <p>Department Dashboard</p>
  <a href="dept_signOut.php"><img id="signOut" src="images/signOut-removebg-preview.png" alt="Sign Out"></a>
</header>

<div class="container">
    <div class="sideBar">
        <div class="profileImg">
            <img src="images/dept_admin.png" alt="">
    ...
    <div class="complaintSection">
        ...
        <form class="two-column-form" method="POST" id="deptForm">
    ...
    ?>
            ...
            <div class="form-group">
                ...
                <textarea name="solution" id="solutionText"></textarea>
            </div>
...
function showModal(msg, type) {
  let imgSrc = '';
  if(type === 'success') imgSrc = 'images/check.png';
  else if(type === 'error') imgSrc = 'images/cross.png';
...
document.getElementById('deptForm').addEventListener('submit', function(e) {
    var solution = document.getElementById('solutionText').value.trim();
    if (!solution) {
        showModal('Please enter a solution before submitting.', 'error');
        e.preventDefault();
    }
});
</script>
